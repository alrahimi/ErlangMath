...
element_set(ElementColumn, ElementRow, Value, Matrix) ->
    {Width, Height} = matrixMorgana:dimensions(Matrix),
    matrixMorgana:new(Width, Height, fun(Column, Row, _, _) ->
            case (Column == ElementColumn) andalso (Row == ElementRow) of
                true ->
                    Value;
                false ->
                    matrixMorgana:element_at(Column, Row, Matrix)
            end
        end).
...
multiply(A, B) ->
    ...
    matrixMorgana:new(Count, length(A), fun(Column, Row, _, _) ->
...
sum_product(Count, Column, Row, A, B, Total) ->
    Product = matrixMorgana:element_at(Count, Row, A) * matrixMorgana:element_at(Column, Count, B),
...
scalar_multiply(Matrix, Scalar) ->
    matrixMorgana:new(length(lists:nth(1, Matrix)), length(Matrix), fun(Column, Row, _, _) ->
...
add(A, B) ->
    matrixMorgana:new(length(lists:nth(1, A)), length(A), fun(Column, Row, _, _) ->
...
maximise_assignment(Matrix) ->
    ...
    {Width, Height} = matrixMorgana:dimensions(Matrix),
    [Row | PreparedMatrix] = matrixMorgana:row_sort(fun({_,X}, {_,Y}) -> X > Y end,
        matrixMorgana:new(Width, Height, 
            fun(Column, Row, Columns, _) ->
                    {1 bsl (Columns - Column), matrixMorgana:element_at(Column, Row, Matrix)}
            end
        )
    ),
...
maximise_assignment_dynamic(Matrix) ->
    {Width, Height} = matrixMorgana:dimensions(Matrix),
    PreparedMatrix = matrixMorgana:new(Width, Height, 
        fun(Column, Row, Columns, _) ->
            {1 bsl (Columns - Column), matrixMorgana:element_at(Column, Row, Matrix)}
        end
    ),
